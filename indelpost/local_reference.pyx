...
cdef class UnsplicedLocalReference:
    ...
    def fetch_ref_seq(self, target_pos):
        ...
        self.left_len = target_pos - max(0, target_pos - self.window * 3)
        return self.get_ref_seq(max(0, target_pos - self.window * 3), min(target_pos + self.window * 3, self.ref_len))
